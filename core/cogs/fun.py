import urllib, json, discord, random
...
class Fun(commands.Cog):
    ...
    @commands.command(help="Random Jokes")
    async def joke(self, ctx):
        jokeapi = urllib.request.urlopen('https://some-random-api.ml/joke')
        jokedata = json.load(jokeapi)

        joke = jokedata['joke']

        embed = discord.Embed(title="Joke", description=f'{joke}', colour=0xb27b56)
        await ctx.send(embed=embed)
